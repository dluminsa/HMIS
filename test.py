   ...
   # st.session_state.fac = None
    ...
    if file is not None and not st.session_state.reader:
        ...
        'TI  ': 'TI', 'TO  ':'TO', 'DD  ': 'DD', 'AG  ':'AG', 'GD  ':'GD', 'NTO  ': 'NTO'})#, 'TPT ': 'TPT'})
                    df = df.rename(columns= {'ART ':'ART',  'AS ':'AS', 'RD ':'RD', 'RD1 ':'RD1', 'RD2 ':'RD2', 'VD ':'VD', 'FE ':'FE', 'LD ': 'LD', 'ARVD ': 'ARVD',
                            'TI ': 'TI', 'TO ':'TO', 'DD ': 'DD', 'AG ':'AG', 'GD ':'GD',  'NTO ': 'NTO'})#, 'TPT  ': 'TPT'})
                    columns = ['ART','AG', 'GD','AS', 'VD', 'RD','TO', 'TI', 'DD', 'FE','LD', 'RD1', 'RD2', 'ARVD', 'NTO', 'DSD','ARVL','CD', 'WT']
    ...
    if st.session_state.reader:
        ...
        'TI  ': 'TI', 'TO  ':'TO', 'DD  ': 'DD', 'AG  ':'AG', 'GD  ':'GD',  'NTO  ': 'NTO'})#, 'TPT ': 'TPT'})
                        ...
                        uploaded = st.session_state.df[['ART','AS', 'AG','VD', 'RD','GD','TO', 'TI', 'DD', 'FE','LD', 'RD1', 'RD2', 'ARVD', 'NTO', 'WT']].copy()#,'TPT']].copy()
            ...
            #             #Clearing NaT from te dates
                       ...
                       #LAST ENCOUTER TO DATES
                        ...
                        df['AGEBND'] = df['AG'].apply(vlager)
                        df['WT'] = pd.to_numeric(df['WT'], errors='coerce')
                        df['WTBD'] = df['WT'].apply(weigh)
                        daty = {
...
##################################################################################################################################
        ...
        ##A             
                        ...
                        if currlost.shape[0] ==0:
                            ...
                            dsdl = pd.pivot_table(currlost,columns='DSDM',values='ART', aggfunc='count',fill_value=0).reset_index()
                            dsdl = dsdl.rename(columns={'index': 'STATUS'})
                            dsdl['STATUS'] = dsdl['STATUS'].astype(str)
                            dsdl['STATUS'] = dsdl['STATUS'].str.replace('ART', 'LOST')
                        ...
                        #VL AUDIT
                        dfaud = dfcur.groupby('AGEBND')['ART'].size().reset_index()
                        dfaud = dfaud.rename(columns={'ART':'ACTIVE'})
                        #PALD
                        dfcur['WT'] = pd.to_numeric(dfcur['WT'], errors='coerce')
                        pald = dfcur[((dfcur['WT']> 5.9) & (dfcur['WT']<25))].copy()
                        if pald.shape[0]>0:
                            dfpald = pald.groupby('WTBD').size().reset_index(name='TOTAL')
                        else:
                            dfpald = pd.DataFrame()

                        #EXTRA INDICATORS 
                        dsd2 = pd.pivot_table(dfcur,columns='DSDM',values='ART', aggfunc='count',fill_value=0).reset_index()
                        dsd2 = dsd2.rename(columns={'index': 'STATUS'})
                        dsd2['STATUS'] = dsd2['STATUS'].astype(str)
                        dsd2['STATUS'] = dsd2['STATUS'].str.replace('ART', 'ACTIVE')
                        # #VL SECTION 
                        #ELIGIBLE FOR VL
                        dfcur[['Ayear', 'Amonth']] = dfcur[['Ayear', 'Amonth']].apply(pd.to_numeric, errors='coerce')
                        has = dfcur[((dfcur['Ayear'] < vayear) | ((dfcur['Ayear'] == vayear) & (dfcur['Amonth']<vamonth)))].copy()
                        dfhc29_elig = has[['ART', 'AGE BAND', 'GD']].copy()
                        dsdv = pd.pivot_table(has,columns='DSDM',values='ART', aggfunc='count',fill_value=0).reset_index()
                        dsdv = dsdv.rename(columns={'index': 'STATUS'})
                        dsdv['STATUS'] = dsdv['STATUS'].astype(str)
                        dsdv['STATUS'] = dsdv['STATUS'].str.replace('ART', 'BELIG')
                        dsd2 = pd.concat([dsd2,dsdv])
                        dr = has.shape[0]
                        

                        #BLED
                        has[['Vyear', 'Vmonth']] = has[['Vyear', 'Vmonth']].apply(pd.to_numeric, errors='coerce')
                        notb = has[((has['Vyear'] < vyeara) | ((has['Vyear'] == vyeara) & (has['Vmonth']<= vmonth)))].copy()
                        ...
                        drck = dr - dra - drb
                        if drck !=0:
                             st.warning('ELIGIBLE FOR VL + BLED + NOTBLED NOT EQUAL TO ACTIVE, CHECK YOUR VL DATES')
                             st.stop()

                        has[['Vyear', 'Vmonth']] = has[['Vyear', 'Vmonth']].apply(pd.to_numeric, errors='coerce')
                        bld = has[((has['Vyear'] > cyear) | ((has['Vyear'] == cyear) & (has['Vmonth']>= fmonth)))].copy()
                        dade = pd.concat([notb, bld])

                        dfaude = dade.groupby('AGEBND')['ART'].size().reset_index()
                        dfaude = dfaude.rename(columns={'ART':'ELIG'})
                        dfaud = pd.merge(dfaud, dfaude, on='AGEBND', how='left')

                        dfaudb = bld.groupby('AGEBND')['ART'].size().reset_index()
                        dfaudb = dfaudb.rename(columns={'ART':'BLED'})
                        dfaud = pd.merge(dfaud, dfaudb, on='AGEBND', how='left')
                        
                        dsdb = pd.pivot_table(has,columns='DSDM',values='ART', aggfunc='count',fill_value=0).reset_index()
                        dsdb = dsdb.rename(columns={'index': 'STATUS'})
                        dsdb['STATUS'] = dsdb['STATUS'].astype(str)
                        dsdb['STATUS'] = dsdb['STATUS'].str.replace('ART', 'BLED')
                        dsd2 = pd.concat([dsd2,dsdb]) 
                        dfhc29_test = has[['ART', 'AGE BAND', 'GD']].copy()
                        has['VR'] = pd.to_numeric(has['VR'], errors = 'coerce')
                        has = has[has['VR']<1000].copy()
                        dsds = pd.pivot_table(has,columns='DSDM',values='ART', aggfunc='count',fill_value=0).reset_index()
                        dsds = dsds.rename(columns={'index': 'STATUS'})
                        dsds['STATUS'] = dsds['STATUS'].astype(str)
                        dsds['STATUS'] = dsds['STATUS'].str.replace('ART', 'SUP')
                        dsd2 = pd.concat([dsd2,dsds])
                        if currlost.shape[0] > 0:
                             dsd2 = pd.concat([dsd2, dsdl])
                        else:
                             dsd2 = dsd2.copy()
...
######################################################24 MONTHS
    ...
    if st.session_state.reader:# and st.session_state.df:
            ...
            extra = st.radio("GENEARATE EXTRA INDICATORS", ('Yes', 'No'), index=None)
            if not extra:
                st.stop()
            elif extra == 'Yes':
                 path = os.path.join(outl,facility+'.csv')
                 dsd2.to_csv(path, index=False)
                 path2 = os.path.join(outv,facility+'.csv')
                 dfaud.to_csv(path2, index=False)
                 path3 = os.path.join(outp,facility+'.csv')
                 dfpald.to_csv(path3, index=False)
            else:
                 pass
            proc = st.radio("Should we proceed with Pivot tables for HTN, DM and MH?", ('Yes', 'No'), index=None)
            if not proc:
                st.stop()
            else:
                        datyn = {
                        ...
                        dfcompp = pd.DataFrame(datyn)
                        htn = df[['AGE BAND', 'HTN', 'ART', 'GD']].copy()

                        dm = df[['AGE BAND', 'DM', 'ART', 'GD']].copy()
                        mh = df[['AGE BAND', 'MH', 'ART', 'GD']].copy()
                        #ma = df[['AGE BAND', 'MA', 'ART', 'GD']].copy()
                        ...
                        h2map = {
                            ...
                            'Newly Diagnosed and on lifestyle modification': 'LFT',
                            'Known Hypertensive, not controlled and on lifestyle modification':'LFT',
                            'Known Hypertensive and Controlled and on lifestyle modification and Medication':'LFT',
                            'Known Hypertensive and Controlled and on lifestyle modification':'LFT',
...
#MH DATA  MENTAL ILLNESS 3333333333333333333333333333333333333333
                        m1map = {
                        ...
                        m2map = {
                            ...
                            'Known mental illness on Psychotherapy only':'LFT',
                        }
...
# #MH DATA  ALCOHOL 3333333333333333333333333333333333333333
#                         ma1map = {
#                             'Known mental illness on Psychotherapy and Medication':'Known',
#                             'Has Signs and Symptoms on Brief Intervention': 'Newly',
...
#                         #     'Known Diabetic and controlled on medication and lifestyle modification':'NCT',
#                         # }
#                         ma['SCR'] = ma['MA'].map(ma1map)
#                         ma['RX'] = ma['MA'].map(ma2map)

#                         mascr = ma[ma['SCR'].isin(['Known', 'Newly', 'Refered'])].copy()
#                        # mdkcr = mh[mh['RX'].isin(['LF', 'LFT'])].copy()
                       
   
    # if facility:# and st.session_state.dfw:
            ...
            with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                        ...
                        hc07 = screened.groupby(['AGE BAND', 'GD']).size().unstack(fill_value=0)
                        hc07 = hc07.reindex(columns=['M', 'F'], fill_value=0)
                        hc07 = hc07.reset_index()
                        hc07 = pd.merge(dfcompp, hc07, on='AGE BAND', how='left')
                        hc07['M'] = hc07['M'].fillna(0)
                        hc07['F'] = hc07['F'].fillna(0) 
                    
                        hc07['SORT'] = hc07['AGE BAND'].map(mapperp)
                        hc07['SORT'] = pd.to_numeric(hc07['SORT'], errors='coerce')
                        hc07 = hc07.sort_values(by ='SORT')
                        hc07 = hc07.drop(columns='SORT')
                        hc07.to_excel(writer, sheet_name="SCREENED", index=False)
###################################################################################################
                        hc06 = nscr.groupby(['SCR', 'AGE BAND', 'GD']).size().unstack('GD', fill_value=0)
                        hc06 = hc06.reindex(columns=['M', 'F'], fill_value=0)
                        hc06 = hc06.reset_index()
                        # hc06 = pd.merge(dfcompp, hc06, on='AGE BAND', how='left')
                        ...
                        hc06['SORT'] = hc06['AGE BAND'].map(mapperp)
                        ...
                        hc08 = kscr.groupby(['RX', 'AGE BAND', 'GD']).size().unstack('GD', fill_value=0)
                        hc08 = hc08.reindex(columns=['M', 'F'], fill_value=0)
                        hc08 = hc08.reset_index()
                        ...
                        hc08['M'] = hc08['M'].fillna(0)
                        hc08['F'] = hc08['F'].fillna(0)
                        hc08['SORT'] = hc08['AGE BAND'].map(mapperp)
                        mapperc = {'LF':1, 'LFT':2}
    ...
    #######################################################################################
                        ...
                        hc09 = cscr.groupby(['CTR', 'AGE BAND', 'GD']).size().unstack('GD', fill_value=0)
                        hc09 = hc09.reindex(columns=['M', 'F'], fill_value=0)
                        hc09 = hc09.reset_index()
                       
                        hc09['M'] = hc09['M'].fillna(0)
                        hc09['F'] = hc09['F'].fillna(0) 
                        hc09['SORT'] = hc09['AGE BAND'].map(mapperp)
                        mapperc = {'CT':1, 'NCT':2}
...
##########################################3DIABETES
                        hc06 = ndscr.groupby(['SCR', 'AGE BAND', 'GD']).size().unstack('GD', fill_value=0)
                        hc06 = hc06.reindex(columns=['M', 'F'], fill_value=0)
                        hc06 = hc06.reset_index()
                        # hc06 = pd.merge(dfcompp, hc06, on='AGE BAND', how='left')
                        hc06['M'] = hc06['M'].fillna(0)
                        ...
                        hc08 = ndscr.groupby(['RX', 'AGE BAND', 'GD']).size().unstack('GD', fill_value=0)
                        hc08 = hc08.reindex(columns=['M', 'F'], fill_value=0)
                        hc08 = hc08.reset_index()
                        ...
                        hc09 = ndccr.groupby(['CTR', 'AGE BAND', 'GD']).size().unstack('GD', fill_value=0)
                        hc09 = hc09.reindex(columns=['M', 'F'], fill_value=0)
                        hc09 = hc09.reset_index()
...
##########################################MENTAL HEALTH
                        hc06 = mdscr.groupby(['SCR', 'AGE BAND', 'GD']).size().unstack('GD', fill_value=0)
                        hc06 = hc06.reindex(columns=['M', 'F'], fill_value=0)
                        hc06 = hc06.reset_index()
                        # hc06 = pd.merge(dfcompp, hc06, on='AGE BAND', how='left')
                        hc06['M'] = hc06['M'].fillna(0)
                        ...
                        hc08 = mdscr.groupby(['RX', 'AGE BAND', 'GD']).size().unstack('GD', fill_value=0)
                        hc08 = hc08.reindex(columns=['M', 'F'], fill_value=0)
                        hc08 = hc08.reset_index()
            ...
            output.seek(0)    
            # facility = st.text_input('FACILITY NAME', key = 'f2')
            # if not facility:
            #                  st.stop()
            # else:
            st.download_button(
                                label="📥 DOWNLOAD NCD PIVOT TABLES",
                                data=output,
